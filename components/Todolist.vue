  ...
  <div class="h-screen min-w-max flex flex-col items-center justify-center">
    <section>
      <form
        ...
        class="border w-72 border-gray-800 rounded-md flex justify-between"
      >
        <input
          type="text"
          placeholder="Enter todo "
          v-model="newTodo"
          class="w-64 h-8 border-none border-transparent bg-transparent"
        />
        ...
        <button type="submit" class="border-l w-11 h-8 border-gray-800 bg-gray-200">
          <svg
            ...
            stroke-width="5"
            stroke="white"
            class="w-6 h-6 ml-2 bg-gray-800 rounded-full"
          >
            <path
          ...
          >
            <span class="flex">
              <label for="checkbox" class="border-none"
                ><input type="checkbox" :value="todo.id" v-model="checked" />
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="4"
                  stroke="currentColor"
                  :class="{ 'text-lime-700': checked == true }"
                  class="w-4 h-4"
                >
                  <path
                    stroke-linecap="round"
              ...
              {{ todo.title }}</span
            >
            <button v-if="checked" @click="removecheckedTodos(todo.id)">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
